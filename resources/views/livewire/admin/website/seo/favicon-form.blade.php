    ...
    <div class="card shadow">
        <div class="card-body">
            ...
            <form wire:submit.prevent="submit">
                ...
                <div x-data="{ uploading: false, progressFavicon: 0 }" x-on:livewire-upload-start="uploading = true"
                    x-on:livewire-upload-finish="uploading = false" x-on:livewire-upload-error="uploading = false"
                    x-on:livewire-upload-progress="progressFavicon = $event.detail.progress">
                    ...
                    @if ($file)
                        <div class="mt-3 border border-indigo-500 bg-indigo-500 p-2">
                            <h4 class="text-white text-center">Favicon Preview</h4>
                        </div>
                    ...
                    @else
                        @if ($seo->favicon_file_path)
                            <div class="mt-3 border border-indigo-500 bg-indigo-500 p-2">
                                <h4 class="text-white text-center">Current Favicon</h4>
                            </div>

                            <div class="flex justify-center ">
                                <img class="w-10 my-5 border-gray-500 border rounded"
                                    src="{{ asset($seo->favicon_file_path) }}?{{ time() }}" alt="">
                            </div>
                        @endif

                        <div class="flex justify-center mt-8 mb-6">
                            <label for="file-upload-favicon"
                        ...
                        <div>
                            <input wire:model.live="file" type="file" class="hidden" id="file-upload-favicon"
                                name="file-upload-favicon" accept=".png{{-- ,.ico --}}">
                        </div>
                    @endif
                    <!-- Current Preview OpenGraph Or Preview OpenGraph Upload -->
                    ...
                    <div x-cloak x-show="uploading" class="mb-8 mt-8">
                        <div class="flex justify-center">
                            ...
                            <span x-text="progressFavicon"></span>
                            <span>
                                %
                            </span>
                        </div>
