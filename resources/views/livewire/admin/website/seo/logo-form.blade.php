    ...
    <div class="card shadow">
        <div class="card-body">
            ...
            <div class="my-3 flex justify-center">
                <a class="btn bg-indigo-600 text-white border-indigo-600 hover:bg-indigo-800 hover:border-indigo-800 active:bg-indigo-800 active:border-indigo-800 focus:outline-none  focus:ring-indigo-300 (!$isFormValid) disabled:bg-red-300"
                    target="_blank" href="https://www.photopea.fr/">
                    Generate Svg files with Photopea
                </a>
            </div>
                ...
                <div x-data="{ uploading: false, progressLogo: 0 }" x-on:livewire-upload-start="uploading = true"
                    ...
                    @if ($file)
                        <div class="mt-3 border border-indigo-500 bg-indigo-500 p-2">
                            <h4 class="text-white text-center">Logo Preview</h4>
                        </div>
                    ...
                    @else
                        @if ($seo->logo_file_path)
                            <div class="mt-3 border border-indigo-500 bg-indigo-500 p-2">
                                <h4 class="text-white text-center">Current Logo</h4>
                            </div>

                            <div class="flex justify-center ">
                                <img class="w-32 my-5 border-gray-500 border rounded"
                                    src="{{ asset($seo->logo_file_path) }}?{{ time() }}" alt="">
                            </div>
                        @endif
                        ...
                        <div>
                            <input wire:model.live="file" type="file" class="hidden" id="file-upload-logo"
                                name="file-upload-logod" accept=".svg">
                        </div>
                    @endif
                    <!-- Current Preview OpenGraph Or Preview OpenGraph Upload -->
